...
import org.apache.hudi.{DataSourceReadOptions, IncrementalRelation, MergeOnReadIncrementalRelation, SparkSqlAdapterSupport}
...
class HoodieStreamSource(
  ...
  extends Source with Logging with Serializable with SparkSqlAdapterSupport {
    ...
    if (startOffset == endOffset) {
      ...
      val rdd = tableType match {
        case HoodieTableType.COPY_ON_WRITE =>
          val serDe = sparkSqlAdapter.createSparkRowSerDe(RowEncoder(schema))
