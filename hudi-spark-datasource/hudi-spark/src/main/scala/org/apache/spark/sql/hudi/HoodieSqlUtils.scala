...
object HoodieSqlUtils {
  ...
  def removeMetaFields(schema: StructType): StructType = {
    ...
    StructType(schema.fields.filterNot(f => metaFields.contains(f.name)))
  }
  ...
  /**
    * Remove the stars from the path.
    * @param path
    * @return
    */
  def removeStarFromPath(path: String): String = {
    if (path == null) {
      null
    } else {
      var newPath = if (path.endsWith("/")) path.substring(0, path.length - 1) else path
