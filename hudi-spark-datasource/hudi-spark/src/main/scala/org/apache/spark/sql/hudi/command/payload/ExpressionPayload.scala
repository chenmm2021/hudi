...
class ExpressionPayload(record: GenericRecord,
  ...
  private def initWriteSchemaIfNeed(properties: Properties): Unit = {
    if (writeSchema == null) {
      ValidationUtils.checkArgument(properties.containsKey(HoodieWriteConfig.WRITE_SCHEMA),
        s"Missing ${HoodieWriteConfig.WRITE_SCHEMA}")
      writeSchema = new Schema.Parser().parse(properties.getProperty(HoodieWriteConfig.WRITE_SCHEMA))
    }
  }
